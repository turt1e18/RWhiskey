...
import { Cloud, Heart, Info, Thermometer, MessageSquare, Utensils, Quote } from "lucide-react";
      ...
      >
        ...
        <header className="border-b-2 border-double border-[#3e2616]/40 pb-5">
          ...
          <h2 className="font-serif text-2xl md:text-3xl text-[#3e2616] font-black leading-tight">
            {whisky.whiskyName || (whisky as any).whiskeyName || (whisky as any).drinkName || (whisky as any).nameKo || "Unknown Item"}
          </h2>
          <p className="text-sm md:text-base text-[#3e2616]/70 font-serif italic font-bold">
            {whisky.whiskyNameEn || (whisky as any).whiskeyNameEn || (whisky as any).drinkNameEn || (whisky as any).nameEn || ""}
          </p>
        ...
        <div className="space-y-4 flex-1">
          <Row icon={<Cloud className="h-4 w-4" />} label="날씨" value={whisky.weatherValue} />
          <Row icon={<Heart className="h-4 w-4" />} label="기분" value={whisky.moodValue} />
          <Row icon={<Thermometer className="h-4 w-4" />} label="선호 도수" value={whisky.abvValue} />
          <Row icon={<MessageSquare className="h-4 w-4" />} label="추가 요청" value={whisky.additionalValue} />
          <Row icon={<Utensils className="h-4 w-4" />} label="추천 간식" value={whisky.foodName} color="#a8201a" />
          ...
          <div className="mt-6 pt-5 border-t border-dotted border-[#3e2616]/40">
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-widest text-[#3e2616]/60 font-black mb-1">Bartender's Word</p>
...
const Row = ({ 
  ...
  <div className="flex items-start gap-4">
    <span className="mt-1 shrink-0" style={{ color }}>{icon}</span>
    <div className="min-w-0 flex-1">
      <p className="text-[10px] uppercase tracking-widest text-[#3e2616]/60 font-black mb-0.5">{label}</p>
